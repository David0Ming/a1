...
class BaseModel(nn.Module):
	...
	class Dataset(BaseDataset):
		...
		def collate_batch(self, feed_dicts: List[dict]) -> Dict[str, torch.Tensor]:
			feed_dict = dict()
			
			# 检查 feed_dicts 是否非空并且每个元素都是字典
			if not feed_dicts or not isinstance(feed_dicts[0], dict):
				raise ValueError("Expected feed_dicts to be a non-empty list of dictionaries.")

			# 输出 feed_dicts 的内容用于调试
			print("Feed dicts:", feed_dicts)

			for key in feed_dicts[0]:
				# 确保所有元素都是 numpy 数组
				if isinstance(feed_dicts[0][key], np.ndarray):
					# 检查长度是否一致
					lengths = [d[key].shape[0] for d in feed_dicts]
					if len(set(lengths)) != 1:  # 如果长度不一致
				...
				if stack_val.dtype == np.object:  # inconsistent length (e.g., history)
					# 使用 pad_sequence 确保返回的张量正确
					feed_dict[key] = pad_sequence([torch.from_numpy(x) for x in stack_val], batch_first=True)
				else:
					# 转换为 torch.Tensor，并确保类型一致
					feed_dict[key] = torch.from_numpy(stack_val).float()  # 根据需要修改数据类型

			feed_dict['batch_size'] = len(feed_dicts)
			feed_dict['phase'] = self.phase  # 确保 self.phase 正确设置
